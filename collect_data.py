...
from util.carla_util import (
    ...
    draw_image,
    get_font,
    should_continue_or_quit, EventStatus,
)

from lane_detection.camera_geometry import (
    get_intrinsic_matrix,
    project_polyline,
    CameraGeometry,
)

from util.seg_data_util import mkdir_if_not_exist

store_files = True
town_string = "Town04"
camera_geometry = CameraGeometry()
width = camera_geometry.image_width
height = camera_geometry.image_height
...
def plot_map(m, start_location):
    ...
    plt.plot(start_location.x, start_location.y, marker='o', ms=10, mfc='r')
    plt.show()
...
def main():
    ...
    try:
        ...
        plot_map(world_map, start_pose.location)
        ...
        cam_rgb_transform = carla.Transform(
            ...
            carla.Rotation(pitch=-1 * camera_geometry.pitch_deg),
        )
        trafo_matrix_vehicle_to_cam = np.array(
            cam_rgb_transform.get_inverse_matrix()
        )
        bp = blueprint_library.find("sensor.camera.rgb")
        fov = camera_geometry.field_of_view_deg
            ...
            while True:
                while True:
                    if waypoint_none:
                        waypoint_none = False
                        continue
                    status = should_continue_or_quit()
                    if status == EventStatus.Quit:
                        return
                    elif status == EventStatus.Continue:
                        break
                    time.sleep(0.5)
                clock.tick()
                ...
                if not next_waypoints:
                    spawn_waypoint = get_random_spawn_point(world_map)
                else:
